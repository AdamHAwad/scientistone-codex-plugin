...
import { RESEARCHER_TIMEOUT_MESSAGE, RESEARCHER_WAIT_TIMEOUT_MS, callTool, clearMonitorIntegrityCache, handleMessage, monitorSnapshot, stop, updateDraft, waitForResearcher } from "../../plugins/scientist1/mcp/server.mjs";
...
test("the fixed-purpose capacity tools persist a decision without project or study input", async (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "scientist1-mcp-capacity-"));
...
test("capacity approval requires and consumes the one-use token", async (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "scientist1-mcp-capacity-approve-"));
  t.after(() => fs.rmSync(codexHome, { recursive: true, force: true }));
  const configFile = path.join(codexHome, "config.toml");
  put(codexHome, "config.toml", "model = \"test\"\n");
  let capacity = 4;
...
test("monitor verification is single-flight and caches only a non-authoritative unchanged snapshot", async (t) => {
  ...
  const run = fs.mkdtempSync(path.join(os.tmpdir(), "scientist1-monitor-cache-"));
...
test("monitor completes only the exact specialist receipt anchored by a verified checkpoint", async (t) => {
  clearMonitorIntegrityCache();
  t.after(() => clearMonitorIntegrityCache());
  const run = fs.mkdtempSync(path.join(os.tmpdir(), "scientist1-monitor-identity-"));
  t.after(() => fs.rmSync(run, { recursive: true, force: true }));
...
test("the stdio server emits only newline-delimited JSON-RPC and exits with its client", async () => {
  ...
  assert.equal(response.result.serverInfo.name, "scientist1-mcp");
});
...
test("a pending researcher wait does not block the MCP", async (t) => {
  const project = fs.mkdtempSync(path.join(os.tmpdir(), "scientist1-mcp-wait-"));
  const child = spawn(process.execPath, [MCP], { stdio: ["pipe", "pipe", "pipe"] });
  t.after(() => {
    child.kill("SIGTERM");
  ...
  assert.equal((await send("initialize", { protocolVersion: "2025-11-25" })).result.serverInfo.name, "scientist1-mcp");
...
test("a researcher wait pauses after one hour with a saved, resumable draft", async (t) => {
  const project = fs.mkdtempSync(path.join(os.tmpdir(), "scientist1-mcp-timeout-"));
  t.after(() => {
    fs.rmSync(project, { recursive: true, force: true });
  });
...
test("intake files persist, approval attaches a verified run, and discard removes only its draft", async (t) => {
  ...
  const preflight = await fetch(apiUrl(context, "/submit"), {
    method: "OPTIONS",
    headers: { Origin: "https://test.web-sandbox.oaiusercontent.com", "Access-Control-Request-Headers": "x-scientist1-token, content-type" },
  });
  ...
  assert.doesNotMatch(bundledUi, /Discard intake|__SCIENTIST1_MCP_APP__|openai:set_globals|requestHost|ui\/initialize|type="module"/);
  ...
  state = await request(context, "/upload", {
    ...
    headers: {
      ...
      "X-Scientist1-Filename": Buffer.from("observations.csv").toString("base64url"),
    },
  });
