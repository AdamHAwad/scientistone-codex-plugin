...
} from "../../plugins/scientist1/skills/scientist1/scripts/capacity-preflight.mjs";

function home(t) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "scientist1-capacity-"));
...
function state(directory) {
  return JSON.parse(fs.readFileSync(path.join(directory, "scientist1", "capacity-preflight.json"), "utf8"));
}
...
test("a version conflict is retried once without keeping a stale backup", async (t) => {
  ...
  const backups = fs.readdirSync(directory).filter((name) => name.includes("scientist1-backup"));
