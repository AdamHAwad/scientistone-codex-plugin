...
import { aggregateVerdicts, compareInterval, evaluateReproducibilityMetric, executionId, summarize, validatePolicySupport } from "../../plugins/scientist1/skills/scientist1/scripts/i1-interpreter.mjs";
...
test("common I1 interpreter fails closed whenever reproducibility does not pass", () => {
  assert.equal(compareInterval({ lower: -0.1, upper: 0.1 }, { lower: -0.2, upper: 0.2 }), "PASS");
  assert.equal(compareInterval({ lower: -0.2, upper: 0.1 }, { lower: -0.2, upper: 0.2 }), "FAIL");
  assert.equal(compareInterval({ lower: -0.2, upper: 0.2 }, { lower: -0.2, upper: 0.2 }), "FAIL");
  ...
  assert.equal(compareInterval({ lower: 0.1, upper: 0.3 }, { lower: -0.2, upper: 0.2 }), "FAIL");
  assert.equal(aggregateVerdicts(["PASS", "NOT_ASSESSED"]), "NOT_ASSESSED");
  assert.equal(aggregateVerdicts(["PASS", "FAIL", "NOT_ASSESSED"]), "FAIL");
});
...
test("common I1 interpreter accepts the task-adaptive direction and estimand surface", () => {
  for (const direction of ["maximize", "minimize", "target", "signed"]) {
    assert.equal(validatePolicySupport({ schema_version: 2, metrics: [{ id: "score", direction, estimand: { type: "mean", semantics_version: "scientist1_i1_v1", parameters: {} }, repetitions: { canonical: 1, audit: 1, valid_required: 1, canonical_run_ids: ["c1"], audit_run_ids: ["a1"] }, comparison_design: "exact", uncertainty: { computed_by: "frozen_evaluator" } }], decision_rule: { type: "all", primary_metric_ids: ["score"], constraint_metric_ids: [] } }), true);
  }
  assert.throws(() => validatePolicySupport({ schema_version: 2, metrics: [{ id: "score", direction: "maximize", estimand: { type: "quantile", semantics_version: "scientist1_i1_v1", parameters: { probability: 0.25, method: "nearest_rank" } }, repetitions: { canonical: 1, audit: 1, valid_required: 1, canonical_run_ids: ["c1"], audit_run_ids: ["a1"] }, comparison_design: "exact", uncertainty: { computed_by: "frozen_evaluator" } }], decision_rule: { type: "all", primary_metric_ids: ["score"], constraint_metric_ids: [] } }), /linear_type7/i);
  assert.throws(() => validatePolicySupport({ schema_version: 2, metrics: [{ id: "score", direction: "maximize", determinism_class: "deterministic", estimand: { type: "mean", semantics_version: "scientist1_i1_v1", parameters: {} }, repetitions: { canonical: 1, audit: 1, valid_required: 1, canonical_run_ids: ["c1"], audit_run_ids: ["a1"] }, comparison_design: "independent", uncertainty: { computed_by: "frozen_evaluator" } }], decision_rule: { type: "all", primary_metric_ids: ["score"], constraint_metric_ids: [] } }), /deterministic metrics require exact/i);
});
...
test("execution identity and reproducibility decisions are recomputed from frozen evidence", () => {
  ...
  const metric = {
    ...
    failure_policy: { missing_pair: "fail", invalid_run: "fail" },
  ...
  assert.equal(evaluateReproducibilityMetric(metric, large).verdict, "FAIL", "deterministic exact drift must never pass through a relative tolerance");
});
