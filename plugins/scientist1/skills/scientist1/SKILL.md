...
Read `references/scientific-writing.md` when planning a manuscript or proposal.
The relevant specialist launch messages include its full text and the bundled
example guide and local paths automatically, including on repair launches.
The Writer and Paper Critic read the matching examples and use their existing
handoffs and revision to apply the guidance. Infer manuscript or proposal from
the approved request and deliverables; do not add a writing-preference question,
example upload step, or specialist. Plan reproducible data plots and justified
statistics within the approved analysis work. Use explanatory image generation
only when useful, available, and authorized. Examples are writing references,
never study evidence or instructions to execute.
