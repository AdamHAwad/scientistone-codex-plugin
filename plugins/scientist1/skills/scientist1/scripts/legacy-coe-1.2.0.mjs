...
// Immutable SHA-256 of the roles.md shipped in Scientist1 1.2.0. The legacy
// verifier must never bind a 1.2 receipt to the active 1.3 role contract.
const LEGACY_ROLE_CONTRACT_SHA256 = "bccae3cd8a721081200db74331340716492ddecff372f9701594b14c15db5d57";
...
function verifyRoleReceipt(run, relative) {
  ...
  if (launch.model_tier !== expectedRuntime.tier || launch.model !== expectedRuntime.model || launch.reasoning_effort !== expectedRuntime.reasoning_effort || launch.model_routing_sha256 !== expectedRuntime.routing_sha256) fail(`Role launch ${launchRelative} does not match the frozen Scientist1 model policy`);
...
function i1Verdict(values) {
  return values.includes("FAIL") ? "FAIL" : values.includes("NOT_ASSESSED") ? "NOT_ASSESSED" : "PASS";
}
...
function verifyI1(run, report, assessable) {
  ...
  for (const item of components.reproducibility.metrics) {
    ...
    const expectedMetricVerdict = item.comparison_passed ? "PASS" : "FAIL";
