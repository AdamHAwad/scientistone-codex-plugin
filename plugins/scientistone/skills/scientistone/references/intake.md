...
- The local browser and MCP keep draft text, uploaded files, approvals, and monitor state inside the active project. They bind only to loopback, and this content is never sent to a remote ScientistOne service.
- After `wait_for_researcher` returns, read the saved draft with `read_study_setup`. For every upload, resolve `stored_path` beneath `.scientistone/intake/<draft-id>/`, reject symbolic links or escapes, verify byte size and SHA-256, and treat that project-local copy as intake material. This staging step is not approval to start the study or create a run.
...
- A file uploaded in the ScientistOne browser is copied over loopback into the active project. No remote ScientistOne service receives it.
...
The approved question, named inputs, constraints, exclusions, data boundaries, and limits on interpretation form the researcher charter. The outcome operationalization, evaluator, verifier, schemas, hashes, paths, seeds, method details, and other generated implementation choices form a versioned execution contract. Selecting `Approve and start study` grants one durable authorization for all safe, reversible, in-scope execution and repair needed to reach the verified deliverables. Agents repair and re-audit that contract in the same run without asking again. If a direct repair would exceed a fixed charter boundary, preserve the boundary, choose the strongest safe in-scope fallback, and disclose the limitation in the paper. Do not solicit an amended plan. Record an amendment only when the researcher independently supplies one. Start a separate run only when the researcher explicitly requests a different question or preserved fork.
...
- `.git`, dependency caches, build caches, old `scientistone-runs`, and system folders;
...
Treat the browser's approved state as the study's only approval checkpoint. It authorizes autonomous execution through the final verified deliverables, including result-blind or result-aware repairs to generated evaluators, verifiers, methods, environments, and schemas within the approved boundaries. Treat edits and a submitted change note before approval as a request for one revised full plan. Do not create the run until the researcher approves the complete version. After approval, never return to plan review or ask for approval, permission, authority, confirmation, a reply, restart, or resume message.
...
A Contract Auditor PASS confirms request fidelity and a usable evaluation. It does not prove the science. If a proposed contract cannot pass, preserve it, choose the smallest faithful generated-contract repair or safe limited design, and send the revision to a fresh auditor. Repeat until PASS; do not create `attention.md`, stop the run, or ask the researcher to decide.
