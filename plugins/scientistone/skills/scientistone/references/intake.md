...
- `wait_for_researcher` waits for up to one hour. If it returns a saved timeout, close the same built-in browser tab, relay its researcher-facing message, and end the turn without polling or advancing the study. Resume the latest draft through `start_study_setup` only after the researcher asks to continue.
...
Treat the browser's approved state as approval. Treat edits and a submitted change note as a request for one revised full plan. Do not create the run until the researcher approves the complete version.
...
A Contract Auditor PASS confirms request fidelity and a usable evaluation. It does not prove the science. If a faithful repair cannot pass, write `attention.md` and stop.
