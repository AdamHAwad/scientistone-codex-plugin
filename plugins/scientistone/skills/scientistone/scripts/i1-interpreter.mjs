...
const VERDICTS = new Set(["PASS", "FAIL", "INCONCLUSIVE", "NOT_ASSESSED"]);
...
function estimandOptions(estimand) {
  if (estimand?.semantics_version !== "scientistone_i1_v1") throw new Error(`${estimand?.type ?? "unknown"} estimand must declare semantics_version scientistone_i1_v1.`);
...
function compareInterval(interval, bounds) {
  ...
  if (upper < allowedLower || lower > allowedUpper) return "FAIL";
  return "INCONCLUSIVE";
}
...
function evaluateReproducibilityMetric(metric, observed) {
  ...
  const comparisonVerdict = observed.interval === null ? "INCONCLUSIVE" : compareInterval(observed.interval, metric.equivalence_margin);
  ...
  const missingOutcome = metric.failure_policy.missing_pair === "fail" ? "FAIL" : "INCONCLUSIVE";
  const invalidOutcome = metric.failure_policy.invalid_run === "fail" ? "FAIL" : "INCONCLUSIVE";
  let verdict;
  const failureVerdicts = [audit.missing ? missingOutcome : null, audit.invalid ? invalidOutcome : null].filter(Boolean);
  if (failureVerdicts.includes("FAIL")) verdict = "FAIL";
  else if (failureVerdicts.length) verdict = "INCONCLUSIVE";
  else if (!enough || !noisePassed || !observed.environment_passed) verdict = "INCONCLUSIVE";
  else if (metric.determinism_class === "deterministic" && ["exact", "paired"].includes(metric.comparison_design) && observed.audit_runs.some((run, index) => run.value !== observed.canonical_runs[index].value)) verdict = "INCONCLUSIVE";
