...
function pathsFor(options = {}) {
  ...
  const stateDirectory = path.join(codexHome, "scientistone");
  return {
    codexHome,
...
function assertUnderRoot(root, target) {
  ...
  const error = new Error("ScientistOne capacity state must remain inside the resolved Codex home.");
...
function assertNoSymlinkComponents(root, target) {
  ...
  for (const component of relative.split(path.sep).filter(Boolean)) {
    ...
    if (fs.lstatSync(current).isSymbolicLink()) {
      const error = new Error("ScientistOne will not mutate symlink-managed Codex configuration or preference paths.");
      error.code = "S1_MANAGED_PATH";
      throw error;
    }
  }
  ...
  while (descriptor === undefined) {
    try {
      ...
      if (Date.now() >= deadline) {
        const busy = new Error("Another ScientistOne capacity preflight is already running.");
...
function codexConfigRequest(method, params, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdin.write(`${JSON.stringify({ id: 0, method: "initialize", params: { clientInfo: { name: "scientistone", title: "ScientistOne", version: "1.2.0" } } })}\n`);
  });
}
...
function backupName(file, now) {
  ...
  return `${file}.scientistone-backup-${stamp}-${process.pid}`;
}
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    if (effectiveCapacity(validated) !== TARGET_CONCURRENCY || managedEffectiveCapacity(validated)) {
      try { await appServerRollback(options, userLayer, response.version); } catch (rollbackError) {
        throw new AggregateError([rollbackError], `ScientistOne could not safely roll back the Codex setting. Recovery backup: ${backupPath ?? "none"}.`);
      }
