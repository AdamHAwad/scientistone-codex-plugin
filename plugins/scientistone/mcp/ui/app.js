...
const steps = [
  ...
  },
  {
    ...
    help: "State anything S1 must not do or claim. The plan will also define a negative or inconclusive result.",
    key: "constraints",
...
async function api(pathname, options = {}) {
  ...
  headers.set("X-ScientistOne-Token", token);
...
function brandBar() {
  return `
    <header class="brand-bar">
      <div class="brand"><img src="${assetUrl("/logo.svg")}" alt=""><span>ScientistOne</span></div>
    </header>`;
}
  ...
  try {
    ...
    for (const file of files) {
      ...
      intake = await api(`/api/intake/upload?${query}`, {
        method: "POST",
        headers: { "Content-Type": file.type || "application/octet-stream", "X-ScientistOne-Filename": filenameHeader(file.name) },
        body: file,
      });
...
const editableReviewFields = ["question", "objective", "materials", "prior_work", "evaluation", "requirements", "negative_or_inconclusive", "deliverables", "study_plan_markdown"];
    ...
    <div class="review-layout">
      <section class="review-copy" aria-labelledby="review-title">
        ...
        <div class="review-sections">
          ...
          ${reviewSection("negative_or_inconclusive", "What counts as a negative or inconclusive result", review.negative_or_inconclusive)}
...
function renderFatal(error, retry) {
  ...
  app.innerHTML = appShell(`
    <section class="waiting">
      <h1>ScientistOne could not load this view</h1>
...
if (!token) {
  renderFatal(new Error("The local ScientistOne session has expired. Reopen it from Codex."), () => location.reload());
} else if (view === "intake" && project && draftId) {
  loadIntake();
} else if (view === "run" && runPath) {
  loadMonitor();
} else {
  renderFatal(new Error("This ScientistOne link is incomplete. Reopen the study from Codex."), () => location.reload());
}
